...
class I_DBScriptSource():
    def __init__(self, *args, **kwargs) -> None:
        ...
        # CREATE ADMIN
        self.script_insert_backoffice_admin = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_10_insert_backoffice_admin.sql")

        # CREATE BACKUSER
        self.script_insert_device_operation_supervisor = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_20_insert_device_operation_supervisor.sql")
        self.script_update_agent_set_device_operation_supervisor_id = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_21_set_device_operation_supervisor_id.sql")

        # CREATE CUSTOMER
        self.script_insert_customer = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_30_insert_customer.sql")
        self.script_update_agent_set_customer_id = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_31_set_customer_id.sql")

        # CREATE DEVICE
        self.script_select_agent__device_operation_supervisor_id = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_40_select_agent__device_operation_supervisor_id.sql")
        self.script_insert_device = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_41_insert_device.sql")
        self.script_update_agent_set_device_id = self._load_sql(self.config._DB_SCRIPTS_DIR / "_1_42_set_device_id.sql")
        ...
        self.script_update_agent__is_enabled = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_12_enable_agent.sql")
        self.script_select_agent__is_enabled = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_13_select_agent__is_enabled.sql")
        self.script_select_device__device_secret = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_14_select_device__device_secret.sql")
        self.script_delete_first_auth_agent = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_15_delete_first_auth_agent.sql")

        # SESSIONS
        # use email to find AGENT_ID and validate AGENT_SECRET
        self.script_select_backoffice_admin_by_email = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_20_select_backoffice_admin_by_email.sql")
        self.script_select_device_operation_supervisor_by_email = self._load_sql(self.config._DB_SCRIPTS_DIR / "_2_21_select_device_operation_supervisor_by_email.sql")
        ...
        # 3 - AGENT INPUTS #
        ####################
        self.script_insert_device_operation = self._load_sql(self.config._DB_SCRIPTS_DIR / "_3_00_insert_device_operation.sql")
